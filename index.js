...
عندما يطلب يانال إجراء مكالمة صادرة:
- إذا أعطاك اسم شخص، استخدم call_contact مباشرة. الأداة تبحث في Google Contacts ثم تبدأ المكالمة.
- إذا أعطاك رقم هاتف، استخدم call_contact مباشرة مع phone_number.
- لا تستخدم find_contact أو make_phone_call في جلسة المتصفح.
- إذا لم يحدد رسالة، يجوز إجراء المكالمة برسالة فارغة، وسيعرّف المساعد عن نفسه عند الرد.
- لا تقل إن الاتصال بدأ إلا إذا أعادت call_contact success=true.
- إذا وجدت عدة جهات اتصال متطابقة، لا تخمّن؛ أخبر يانال أن الاسم غير محدد واطلب توضيحًا.
...
dc.onmessage = async (event) => {
  try {
    ...
    console.log('Realtime event:', response.type, response.name || '', response.call_id || '');

    if (response.type !== 'response.function_call_arguments.done') return;

    if (response.name !== 'call_contact') {
      console.log('Ignoring unknown browser tool:', response.name);
      return;
    }

    const args = JSON.parse(response.arguments || '{}');
    console.log('TOOL CALLED: call_contact', args);

    statusEl.textContent = args.contact_name
    ...
    try {
      ...
      result = { success: false, error: 'Server returned invalid JSON' };
    }

    console.log('TOOL RESULT: call_contact', result);
    ...
    statusEl.textContent = result.success
      ? '✅ بدأ الاتصال'
      : '❌ تعذر الاتصال: ' + (result.error || 'Unknown error');
  } catch (error) {
    console.error('Data channel tool error:', error);
    statusEl.textContent = '❌ خطأ بتنفيذ الأمر: ' + error.message;
  }
};
...
fastify.post('/session', async (request, reply) => {
  ...
  type: 'function',
  name: 'call_contact',
  description: 'Start an outbound call either by Google Contacts name or by direct phone number. Use whenever Yanal asks to call or ring someone.',
  parameters: {
    type: 'object',
    properties: {
      contact_name: { type: 'string', description: 'Google Contacts name, when a name was provided' },
      phone_number: { type: 'string', description: 'Direct phone number, preferably E.164, when a number was provided' },
      message: { type: 'string', description: 'Optional message to deliver on the call' }
    }
  }
}
      ],
...
const outboundMessage = String(message || '');
...
console.log('Message to deliver:', outboundMessage);
    ...
    const call = await twilioClient.calls.create({
      ...
      url: `https://${request.headers.host}/outgoing-call?message=${encodeURIComponent(outboundMessage)}`
    });
